...
#SBATCH --job-name=gw-medieval
#SBATCH --partition=IllinoisComputes
#SBATCH --array=0-11%1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=08:00:00
#SBATCH --output=logs/datagen_medieval_%A_%a.out
#SBATCH --error=logs/datagen_medieval_%A_%a.err
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=jalen.jiang2+slurm@gmail.com

# One task per city, sequential (%1). cdg.py skips existing tiles so re-runs are safe.
...
CITIES_CONFIG="data_pipeline/cities_medieval_organic.yaml"
DATA_OUTPUT="data_medieval_organic/"
CITY=$(python3 -c "import yaml; cfg=yaml.safe_load(open('$CITIES_CONFIG')); print(cfg['cities'][$SLURM_ARRAY_TASK_ID]['name'])")

echo "========================================"
echo "Groundwork Datagen — Medieval Organic"
