...
def get_train_dataset(args, accelerator):
    ...
    # PATCH: cast conditioning_image path column to HF Image feature so PIL loads automatically.
    # imagefolder auto-decodes only file_name -> image; other path columns stay as strings.
    from datasets import Image as _HFImage
    if args.conditioning_image_column and args.conditioning_image_column in dataset["train"].column_names:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                # PATCH: cast cached prompt_embeds (deserialize as fp32 from Arrow) to weight_dtype
                batch["prompt_ids"] = batch["prompt_ids"].to(dtype=weight_dtype)
                batch["unet_added_conditions"] = {k: v.to(dtype=weight_dtype) for k, v in batch["unet_added_conditions"].items()}
